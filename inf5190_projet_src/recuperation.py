...
def indices_donnees_piscine(row):
    ...
    Sortie : liste_indices (list[int]) :
            nombre de colonnes concernees par le champ i
            dans la base de données
    """
    # 12 colonnes dans la table piscines
    liste_indices = [0]*12
    i = 0
    curseur = 0
    while i < (len(row)):
...
def arrangement_ligne(liste_indices, row):
    """
    Entrée : row (list[str]) : ligne du document piscine.csv
             liste_indices (list[int]) :
                représente le nombre de colonnes concernees
                par le champ i dans la base de données
    Sortie : good_row (list[str]) :
                ligne du document piscine.csv avec
                l'adresse dans une seule colonne
    """
...
def importation_donnees_piscines():
    req = requests.get(
        "https://data.montreal.ca/dataset/4604afb7-a7c4-4626"
...
def del_spaces(string):
    ...
    Sortie : nom_arrondissement (str) : le nom de l'arrondissement
                                        sans les espace autour du '-'
    Cela permet d'assurer que les noms d'arrondissement soit
    renseignés identiquement dans les trois tables.
    """
    correct = string
...
def del_unwanted(liste):
    ...
    for i in range(len(liste)):
        ...
        while (
            ...
            (mot[indice_debut] in unwanted
             or mot[indice_debut:indice_debut+1] in unwanted)):
            indice_debut += 1
        indice_fin = len(mot)-1
        while (
            indice_fin >= 0 and
            (mot[indice_fin] in unwanted
             or mot[indice_fin:indice_debut-1] in unwanted)):
            indice_fin -= 1
...
def gestion_donnees_patinoires():
    ...
    for arrondissement in root.findall('arrondissement'):
        ...
        for patinoire in arrondissement.findall('patinoire'):
            ...
            donnees = [nom_pat, nom_arr, date_heure,
