...
class Database:
    ...
    def insert_piscine(self, row):
        ...
        cursor.execute(
            "INSERT INTO piscines(id_uev, type, nom, nom_arrondissement, adresse, propriete, gestion, point_x, point_y, equipement, long, lat)"
            " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
    ...
    def update_piscine(self, row):
        ...
        cursor.execute(
            ...
            "adresse=?, propriete=?, gestion=?, point_x=?, point_y=?, equipement=?, long=?, lat=? "
            "WHERE id_uev = ?;",
            (row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7],
    ...
    def get_glissade(self, nom):
        ...
        return [Glissade(instal[0], instal[1], instal[2],
                       instal[3], instal[4], instal[5], instal[6], instal[7]) 
    ...
    def insert_glissade(self, row):
        ...
        cursor.execute(
            "INSERT INTO glissades(nom, nom_arrondissement, cle_arrondissement, date_maj_arrondissement, ouvert, deblaye, condition)"
            " VALUES (?, ?, ?, ?, ?, ?, ?);",
    ...
    def insert_patinoire(self, row):
        ...
        cursor.execute(
            "INSERT INTO patinoires(nom, nom_arrondissement, date_maj, ouvert, deblaye, arrose, resurface)"
            " VALUES (?, ?, ?, ?, ?, ?, ?);",
            (row[0], row[1], row[2], row[3], row[4], row[5], row[6]))
